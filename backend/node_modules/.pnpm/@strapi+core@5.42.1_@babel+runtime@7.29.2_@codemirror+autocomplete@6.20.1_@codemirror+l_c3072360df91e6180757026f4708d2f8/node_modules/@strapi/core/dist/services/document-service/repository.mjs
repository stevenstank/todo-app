import { curry, assoc, omit, merge, isEmpty, pick } from 'lodash/fp';
import { validate, contentTypes, async, createModelCache, errors } from '@strapi/utils';
import { wrapInTransaction } from './common.mjs';
import { defaultStatus as defaultStatusCurry, statusToLookup as statusToLookupCurry, filterDataPublishedAt as filterDataPublishedAtCurry, setStatusToDraft as setStatusToDraftCurry, statusToData as statusToDataCurry, defaultToDraft as defaultToDraftCurry } from './draft-and-publish.mjs';
...
import { omitComponentData, updateComponents, createComponentRelationFilter } from './components.mjs';
import { createEntriesService } from './entries.mjs';
import { pickSelectionParams, ALLOWED_DOCUMENT_ROOT_PARAM_KEYS } from './params.mjs';
...
const createContentTypeRepository = (uid, validator = entityValidator)=>{
   ...
   */ const checkStatus = (params, strict)=>{
        ...
        if (params.status !== 'published' && params.status !== 'draft') {
            throw new errors.ValidationError(`Invalid parameter at 'status'. Expected 'published' or 'draft', received: ${params.status}`);
        }
        return params;
    };
    /**
   ...
   */ const checkLocale = (params, strict)=>{
        ...
        if (typeof params.locale === 'object' && !Array.isArray(params.locale)) {
            throw new errors.ValidationError(`Invalid parameter at 'locale'. Expected a string, array of strings, empty string, null, or undefined; received: object`, {
                received: params.locale
            });
        }
        const validateAndNormalizeOne = (value, path)=>{
            if (typeof value !== 'string') {
                throw new errors.ValidationError(`Invalid parameter at '${path}'. Expected a string, received: ${typeof value}`, {
                    received: value
                });
            }
   ...
   */ const parsePaginationInt = (name, value, strict, spec)=>{
        ...
        if (!valid && strict) {
            ...
            throw new errors.ValidationError(`Invalid parameter at '${name}'. Expected ${expected}, received: ${value}`);
        }
    ...
    const parseWithCount = (value)=>{
        ...
        throw new errors.ValidationError(`Invalid parameter at 'withCount'. Expected a boolean, received: ${typeof value}`);
    };
   ...
   */ const checkPagination = (params, strict)=>{
        ...
        if (hasPage && hasOffset) {
            throw new errors.ValidationError('Invalid pagination parameters. Cannot use both page-based (page, pageSize) and offset-based (start, limit) pagination in the same query.');
        }
   ...
   */ const checkUnrecognizedRootParams = (params, strict)=>{
        if (!strict) {
            return params;
        }
        return pick(ALLOWED_DOCUMENT_ROOT_PARAM_KEYS, params);
    };
    const validateParams = async (params)=>{
        ...
        if (rawStrictParams !== undefined && rawStrictParams !== false && rawStrictParams !== true) {
            throw new errors.ValidationError(`Invalid config.api.documents.strictParams value: "${rawStrictParams}". Expected boolean (true or false).`);
        }
    ...
    async function clone(opts = {}) {
        ...
        const clonedEntries = await async.map(entriesToClone, async.pipe(omit([
            'id',
            'createdAt',
            'updatedAt'
