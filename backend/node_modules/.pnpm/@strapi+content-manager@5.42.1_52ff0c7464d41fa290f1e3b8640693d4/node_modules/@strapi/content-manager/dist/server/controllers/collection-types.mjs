...
var collectionTypes = {
    async find (ctx) {
        ...
        const populate = await getService('populate-builder')(model).populateFromQuery(permissionQuery).populateDeep(1).countRelations({
            toOne: false,
            toMany: true
        }).withPopulateOverride(getPopulateForLocalizations(model)).build();
        ...
        const statusByDocumentId = hasDraftAndPublish ? indexByDocumentId(await documentMetadata.getManyAvailableStatus(model, documents)) : new Map();
    ...
    async findOne (ctx) {
        ...
        const populate = await getService('populate-builder')(model).populateFromQuery(permissionQuery).populateDeep(Infinity).countRelations().withPopulateOverride(getPopulateForLocalizations(model)).build();
...
export { collectionTypes as default };
