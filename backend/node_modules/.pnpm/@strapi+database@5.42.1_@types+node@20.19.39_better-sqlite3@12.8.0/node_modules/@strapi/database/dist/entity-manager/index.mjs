...
import { deleteRelatedMorphOneRelationsAfterMorphToManyUpdate, encodePolymorphicRelation, encodePolymorphicId } from './morph-relations.mjs';
import { hasOrderColumn, hasInverseOrderColumn, isAnyToOne, isBidirectional, isOneToAny } from '../metadata/relations.mjs';
import '../utils/identifiers/index.mjs';
import { deleteRelations, cleanOrderColumns, deletePreviousOneToAnyRelations, deletePreviousAnyToOneRelations } from './regular-relations.mjs';
 ...
 */ async function batchInsertJoinTable(db, joinTableName, rows, trx, options) {
    if (rows.length === 0) return;
    if (trx == null) {
        throw new Error('batchInsertJoinTable requires a transaction so all batches commit or roll back atomically');
    }
...
const createEntityManager = (db)=>{
        ...
        async createMany (uid, params = {}) {
            ...
            try {
                for(let i = 0; i < dataToInsert.length; i += batchSize){
                    ...
                    const chunkResult = await this.createQueryBuilder(uid).insert(chunk).transacting(trx.get()).execute();
                    createdEntries = createdEntries.concat(Array.isArray(chunkResult) ? chunkResult : [
                        chunkResult
                    ]);
                }
                await trx.commit();
                    ...
                    if (isNull(cleanRelationData.set)) {
                        ...
                        if (isPartialUpdate) {
                            ...
                            await batchInsertJoinTable(db, joinTable.name, insert, trx, {
                                ...
                                merge: hasOrderColumn(attribute) ? [
                                    orderColumnName
                                ] : undefined,
     ...
     */ async insertJoinTableRows (joinTableName, rows, trx, options) {
            if (rows.length === 0) return;
            if (trx == null) {
                throw new Error('insertJoinTableRows requires a transaction so all batches commit or roll back atomically');
            }
            const batchSize = db.dialect.getBatchInsertSize();
            for(let i = 0; i < rows.length; i += batchSize){
                const chunk = rows.slice(i, i + batchSize);
