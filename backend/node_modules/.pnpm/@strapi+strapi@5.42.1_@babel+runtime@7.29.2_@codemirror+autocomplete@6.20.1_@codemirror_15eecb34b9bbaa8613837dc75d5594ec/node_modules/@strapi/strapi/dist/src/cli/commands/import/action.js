 ...
 */ var action = (async (opts)=>{
   ...
   */ const engineOptions = {
        ...
        transforms: {
            ...
            entities: [
                {
                    filter: (entity)=>!dataTransfer$1.DEFAULT_IGNORED_CONTENT_TYPES.includes(entity.type)
                }
            ]
        }
    };
    ...
    engine.diagnostics.onDiagnostic(dataTransfer$1.formatDiagnostic('import', opts.verbose));
    ...
    progress.on('transfer::start', async ()=>{
        ...
        await strapiInstance.telemetry.send('didDEITSProcessStart', dataTransfer$1.getTransferTelemetryPayload(engine));
    });
    let results;
    try {
        ...
        await strapiInstance.telemetry.send('didDEITSProcessFinish', dataTransfer$1.getTransferTelemetryPayload(engine));
        await strapiInstance.destroy();
        helpers.exitWith(0, dataTransfer$1.exitMessageText('import'));
    } catch (e) {
        await strapiInstance.telemetry.send('didDEITSProcessFail', dataTransfer$1.getTransferTelemetryPayload(engine));
        helpers.exitWith(1, dataTransfer$1.exitMessageText('import', true));
    }
});
/**
