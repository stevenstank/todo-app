import "./chunk-LK32TJAX.js";

// node_modules/@strapi/admin/dist/admin/admin/src/translations/de.json.mjs
var Analytics = "Analytics";
var Documentation = "Dokumentation";
var Email = "E-Mail";
var Password = "Passwort";
var Provider = "Methode";
var ResetPasswordToken = "Passwort-Token zurücksetzen";
var Role = "Rolle";
var Username = "Benutzername";
var Users = "Benutzer";
var anErrorOccurred = "Ups! Ein unbekannter Fehler ist aufgetreten. Versuche es erneut.";
var noPreview = "Keine Vorschau verfügbar";
var clearLabel = "Zurücksetzen";
var dark = "Dunkel";
var light = "Hell";
var or = "ODER";
var selectButtonTitle = "Auswählen";
var skipToContent = "Zum Inhalt springen";
var submit = "Senden";
var de = {
  Analytics,
  "Auth.components.Oops.text": "Dein Account wurde deaktiviert",
  "Auth.components.Oops.text.admin": "Falls das ein Fehler war, kontaktiere bitte deinen Administrator.",
  "Auth.components.Oops.title": "Ups...",
  "Auth.form.active.label": "Aktiv",
  "Auth.form.button.forgot-password": "E-Mail versenden",
  "Auth.form.button.go-home": "ZURÜCK ZUR STARTSEITE",
  "Auth.form.button.login": "Login",
  "Auth.form.button.login.providers.error": "Durch den ausgewählten Anbieter können wir dich nicht verbinden",
  "Auth.form.button.login.strapi": "Per Strapi einloggen",
  "Auth.form.button.password-recovery": "Passwort zurücksetzen",
  "Auth.form.button.register": "Los geht's",
  "Auth.form.confirmPassword.label": "Passwort bestätigen",
  "Auth.form.currentPassword.label": "Aktuelles Passwort",
  "Auth.form.email.label": "E-Mail",
  "Auth.form.email.placeholder": "z.B. max.mustermann@gmail.com",
  "Auth.form.error.blocked": "Dein Account wurde vom Administrator blockiert.",
  "Auth.form.error.code.provide": "Ungültiger Code.",
  "Auth.form.error.confirmed": "Deine Account E-Mail-Adresse ist nicht bestätigt.",
  "Auth.form.error.email.invalid": "Diese E-Mail-Adresse ist ungültig.",
  "Auth.form.error.email.provide": "Bitte nenne uns deinen Benutzernamen oder deine E-Mail-Adresse.",
  "Auth.form.error.email.taken": "Diese E-Mail-Adresse wird bereits genutzt",
  "Auth.form.error.invalid": "Ungültige Login-Daten.",
  "Auth.form.error.params.provide": "Ungültige Parameter.",
  "Auth.form.error.password.format": "Dein Passwort darf nicht mehr als dreimal das Symbol `$` enthalten.",
  "Auth.form.error.password.local": "Dieser Benutzer hat kein lokales Passwort. Bitte logge dich mithilfe des Providers ein, der bei der Erstellung des Accounts genutzt wurde.",
  "Auth.form.error.password.matching": "Passwörter sind nicht gleich.",
  "Auth.form.error.password.provide": "Bitte gib dein Passwort ein.",
  "Auth.form.error.ratelimit": "Zu viele Versuche, bitte versuche es in einer Minute erneut.",
  "Auth.form.error.user.not-exist": "Diese E-Mail-Adresse ist nicht registriert.",
  "Auth.form.error.username.taken": "Dieser Benutzername ist bereits vergeben",
  "Auth.form.firstname.label": "Vorname",
  "Auth.form.firstname.placeholder": "z.B. Max",
  "Auth.form.forgot-password.email.label": "Gib deine E-Mail ein",
  "Auth.form.forgot-password.email.label.success": "Eine E-Mail wurde erfolgreich verschickt an",
  "Auth.form.lastname.label": "Nachname",
  "Auth.form.lastname.placeholder": "z.B. Mustermann",
  "Auth.form.password.hide-password": "Passwort ausblenden",
  "Auth.form.password.hint": "Das Passwort muss mindestens 8 Zeichen, einen Großbuchstaben, einen Kleinbuchstaben und eine Zahl enthalten",
  "Auth.form.password.show-password": "Passwort einblenden",
  "Auth.form.register.news.label": "Halte mich über die neuen Features und anstehenden Verbesserungen auf dem Laufenden (damit werden die {terms} und die {policy} akzeptiert).",
  "Auth.form.register.subtitle": "Deine Zugangsdaten werden nur verwendet, um dich im Admin Panel einzuloggen. Alle Daten werden in der lokalen Datenbank gespeichert.",
  "Auth.form.rememberMe.label": "Eingeloggt bleiben",
  "Auth.form.username.label": "Benutzername",
  "Auth.form.username.placeholder": "z.B. Max_Mustermann",
  "Auth.form.welcome.subtitle": "Logge dich in deinen Strapi Account ein",
  "Auth.form.welcome.title": "Willkommen!",
  "Auth.link.forgot-password": "Passwort vergessen?",
  "Auth.link.ready": "Bereit für den Login?",
  "Auth.link.signin": "Einloggen",
  "Auth.link.signin.account": "Account bereits vorhanden?",
  "Auth.login.sso.divider": "Oder einloggen mit",
  "Auth.login.sso.loading": "Anbieter werden geladen...",
  "Auth.login.sso.subtitle": "Per SSO einloggen",
  "Auth.privacy-policy-agreement.policy": "Datenschutzerklärung",
  "Auth.privacy-policy-agreement.terms": "Nutzungsbedingungen",
  "Auth.reset-password.title": "Passwort zurücksetzen",
  "Content Manager": "Inhalts-Manager",
  "Content Type Builder": "Inhaltstyp-Editor",
  Documentation,
  Email,
  "Files Upload": "Dateien hochladen",
  "HomePage.head.title": "Startseite",
  "HomePage.header.title": "Hallo {name}",
  "HomePage.header.subtitle": "Willkommen in deinem Administrationsbereich",
  "HomePage.widget.loading": "Widget-Inhalt wird geladen",
  "HomePage.widget.error": "Widget-Inhalt konnte nicht geladen werden.",
  "HomePage.widget.no-data": "Kein Inhalt gefunden.",
  "HomePage.widget.no-permissions": "Du hast nicht die Berechtigung, dieses Widget zu sehen",
  "Media Library": "Medienbibliothek",
  "New entry": "Neuer Eintrag",
  Password,
  Provider,
  ResetPasswordToken,
  Role,
  "Roles & Permissions": "Rollen & Berechtigungen",
  "Roles.ListPage.notification.delete-all-not-allowed": "Manche Rollen konnten nicht gelöscht werden, da sie mit Benutzern verknüpft sind",
  "Roles.ListPage.notification.delete-not-allowed": "Eine Rolle, die mit einem Benutzer verknüpft ist, kann nicht gelöscht werden",
  "Roles.RoleRow.select-all": "Wähle {name} für Mehrfach-Aktionen",
  "Roles.RoleRow.user-count": "Benutzer",
  "Roles.components.List.empty.withSearch": "Es gibt keine Rolle die der Suche ({search}) entspricht...",
  "Settings.PageTitle": "Einstellungen - {name}",
  "Settings.apiTokens.copy.lastWarning": "Kopiere dein API-Token",
  "Settings.apiTokens.ListView.headers.createdAt": "Erstellt am",
  "Settings.apiTokens.ListView.headers.description": "Beschreibung",
  "Settings.apiTokens.ListView.headers.lastUsedAt": "Zuletzt verwendet",
  "Settings.apiTokens.ListView.headers.name": "Name",
  "Settings.apiTokens.ListView.headers.type": "Token-Typ",
  "Settings.apiTokens.addFirstToken": "Deinen ersten API-Token hinzufügen",
  "Settings.apiTokens.addNewToken": "Neuen API-Token hinzufügen",
  "Settings.apiTokens.create": "Neuen API-Token erstellen",
  "Settings.apiTokens.createPage.BoundRoute.title": "Gebundene Route zu",
  "Settings.apiTokens.createPage.permissions.description": "Nur Aktionen, die an eine Route gebunden sind, werden unten aufgeführt.",
  "Settings.apiTokens.createPage.permissions.header.hint": "Wähle die Aktionen der Anwendung oder des Plugins und klicke auf das Zahnradsymbol, um die gebundene Route anzuzeigen",
  "Settings.apiTokens.createPage.permissions.header.title": "Erweiterte Einstellungen",
  "Settings.apiTokens.createPage.permissions.title": "Berechtigungen",
  "Settings.apiTokens.createPage.title": "API-Token erstellen",
  "Settings.apiTokens.description": "Liste der generierten Tokens mit Zugriff auf die API",
  "Settings.apiTokens.emptyStateLayout": "Du hast noch keinen Inhalt...",
  "Settings.apiTokens.regenerate": "Neu generieren",
  "Settings.apiTokens.title": "API-Tokens",
  "Settings.apiTokens.lastHour": "letzte Stunde",
  "Settings.application.customization": "Anpassung",
  "Settings.application.customization.auth-logo.carousel-hint": "Ersetze das Logo auf den Authentifizierungsseiten",
  "Settings.application.customization.carousel-hint": "Ändere das Logo des Admin-Panels (Max dimension: {dimension}x{dimension}, Max file size: {size}KB)",
  "Settings.application.customization.carousel-slide.label": "Logo-Folie",
  "Settings.application.customization.carousel.auth-logo.title": "Auth-Logo",
  "Settings.application.customization.carousel.change-action": "Logo ändern",
  "Settings.application.customization.carousel.menu-logo.title": "Menu-Logo",
  "Settings.application.customization.carousel.reset-action": "Logo zurücksetzen",
  "Settings.application.customization.carousel.title": "Logo",
  "Settings.application.customization.menu-logo.carousel-hint": "Ersetze das Logo in der Hauptnavigation",
  "Settings.application.customization.modal.cancel": "Abbrechen",
  "Settings.application.customization.modal.pending": "Ausstehendes Logo",
  "Settings.application.customization.modal.pending.card-badge": "Bild",
  "Settings.application.customization.modal.pending.choose-another": "Wähle ein anderes Logo",
  "Settings.application.customization.modal.pending.subtitle": "Verwalte das ausgewählte Logo vor dem Hochladen",
  "Settings.application.customization.modal.pending.title": "Logo bereit zum Hochladen",
  "Settings.application.customization.modal.pending.upload": "Logo hochladen",
  "Settings.application.customization.modal.tab.label": "Wie möchtest du deine Assets hochladen?",
  "Settings.application.customization.modal.upload": "Logo hochladen",
  "Settings.application.customization.modal.upload.cta.browse": "Dateien durchsuchen",
  "Settings.application.customization.modal.upload.drag-drop": "Drag & Drop hier oder",
  "Settings.application.customization.modal.upload.error-format": "Falsches Format hochgeladen (nur akzeptierte Formate: jpeg, jpg, png, svg).",
  "Settings.application.customization.modal.upload.error-network": "Netzwerkfehler",
  "Settings.application.customization.modal.upload.error-size": "Die hochgeladene Datei ist zu groß (maximale Dimension: {dimension}x{dimension}, maximale Dateigröße: {size}KB)",
  "Settings.application.customization.modal.upload.file-validation": "Maximale Dimension: {dimension}x{dimension}, Maximale Größe: {size}KB",
  "Settings.application.customization.modal.upload.from-computer": "Von Computer",
  "Settings.application.customization.modal.upload.from-url": "Von URL",
  "Settings.application.customization.modal.upload.from-url.input-label": "URL",
  "Settings.application.customization.modal.upload.next": "Weiter",
  "Settings.application.customization.size-details": "Maximale Dimension: {dimension}x{dimension}, Maximale Dateigröße: {size}KB",
  "Settings.application.description": "Globale Informationen über die Administrationsoberfläche",
  "Settings.application.edition-title": "Aktuelle Version",
  "Settings.application.ee-or-ce": "{communityEdition, select, true {Community Edition} other {Enterprise Edition}}",
  "Settings.application.ee.admin-seats.add-seats": "{isHostedOnStrapiCloud, select, true {Sitze hinzufügen} other {Vertrieb kontaktieren}}",
  "Settings.application.ee.admin-seats.support": "Vertrieb kontaktieren",
  "Settings.application.ee.admin-seats.at-limit-tooltip": "Am Limit: Sitze hinzufügen, um mehr Benutzer einzuladen",
  "Settings.application.ee.admin-seats.count": "<text>{enforcementUserCount}</text>/{permittedSeats}",
  "Settings.application.get-help": "Hilfe",
  "Settings.application.link-pricing": "Alle Preisgestaltungen anzeigen",
  "Settings.application.link-upgrade": "Deine Administrationsoberfläche aktualisieren",
  "Settings.application.node-version": "Node-Version",
  "Settings.application.strapi-version": "Strapi-Version",
  "Settings.application.strapiVersion": "Strapi-Version",
  "Settings.application.title": "Übersicht",
  "Settings.error": "Fehler",
  "Settings.global": "Globale Einstellungen",
  "Settings.permissions": "Administrationsoberfläche",
  "Settings.permissions.auditLogs.action": "Aktion",
  "Settings.permissions.auditLogs.admin.auth.success": "Admin-Anmeldung",
  "Settings.permissions.auditLogs.admin.logout": "Admin-Abmeldung",
  "Settings.permissions.auditLogs.component.create": "Komponente erstellen",
  "Settings.permissions.auditLogs.component.delete": "Komponente löschen",
  "Settings.permissions.auditLogs.component.update": "Komponente aktualisieren",
  "Settings.permissions.auditLogs.content-type.create": "Inhaltstyp erstellen",
  "Settings.permissions.auditLogs.content-type.delete": "Inhaltstyp löschen",
  "Settings.permissions.auditLogs.content-type.update": "Inhaltstyp aktualisieren",
  "Settings.permissions.auditLogs.date": "Datum",
  "Settings.permissions.auditLogs.details": "Protokolldetails",
  "Settings.permissions.auditLogs.entry.create": "Eintrag erstellen{model, select, undefined {} other { ({model})}}",
  "Settings.permissions.auditLogs.entry.delete": "Eintrag löschen{model, select, undefined {} other { ({model})}}",
  "Settings.permissions.auditLogs.entry.publish": "Eintrag veröffentlichen{model, select, undefined {} other {({model})}}",
  "Settings.permissions.auditLogs.entry.unpublish": "Eintrag zurückziehen{model, select, undefined {} other { ({model})}}",
  "Settings.permissions.auditLogs.entry.update": "Eintrag aktualisieren{model, select, undefined {} other { ({model})}}",
  "Settings.permissions.auditLogs.filters.combobox.aria-label": "Suche und wähle eine Option zum Filtern",
  "Settings.permissions.auditLogs.listview.header.subtitle": "Protokolle aller Aktivitäten, die in deiner Umgebung stattgefunden haben",
  "Settings.permissions.auditLogs.not-available": "Audit Logs ist nur im Rahmen eines kostenpflichtigen Tarifs verfügbar. Upgrade, um eine durchsuchbare und filterbare Anzeige aller Aktivitäten zu erhalten.",
  "Settings.permissions.auditLogs.media.create": "Medien erstellen",
  "Settings.permissions.auditLogs.media.delete": "Medien löschen",
  "Settings.permissions.auditLogs.media.update": "Medien aktualisieren",
  "Settings.permissions.auditLogs.payload": "Payload",
  "Settings.permissions.auditLogs.permission.create": "Berechtigung erstellen",
  "Settings.permissions.auditLogs.permission.delete": "Berechtigung entfernen",
  "Settings.permissions.auditLogs.permission.update": "Berechtigung aktualisieren",
  "Settings.permissions.auditLogs.role.create": "Rolle erstellen",
  "Settings.permissions.auditLogs.role.delete": "Rolle löschen",
  "Settings.permissions.auditLogs.role.update": "Rolle aktualisieren",
  "Settings.permissions.auditLogs.user": "Benutzer",
  "Settings.permissions.auditLogs.user.create": "Benutzer erstellen",
  "Settings.permissions.auditLogs.user.delete": "Benutzer löschen",
  "Settings.permissions.auditLogs.user.fullname": "{firstname} {lastname}",
  "Settings.permissions.auditLogs.user.update": "Benutzer aktualisieren",
  "Settings.permissions.auditLogs.userId": "Benutzer ID",
  "Settings.permissions.category": "Berechtigungseinstellungen für die {category}",
  "Settings.permissions.category.plugins": "Berechtigungseinstellungen für das {category} Plugin",
  "Settings.permissions.conditions.anytime": "Jederzeit",
  "Settings.permissions.conditions.apply": "Anwenden",
  "Settings.permissions.conditions.can": "Kann",
  "Settings.permissions.conditions.conditions": "Bedingungen definieren",
  "Settings.permissions.conditions.define-conditions": "Bedingungen definieren",
  "Settings.permissions.conditions.links": "Links",
  "Settings.permissions.conditions.no-actions": "Keine Aktionen",
  "Settings.permissions.conditions.none-selected": "Jederzeit",
  "Settings.permissions.conditions.or": "ODER",
  "Settings.permissions.conditions.when": "Wenn",
  "Settings.permissions.select-all-by-permission": "Wähle alle {label}-Berechtigungen",
  "Settings.permissions.select-by-permission": "Wähle {label}-Berechtigung",
  "Settings.permissions.users.active": "Aktiv",
  "Settings.permissions.users.create": "Benutzer einladen",
  "Settings.permissions.users.email": "E-Mail",
  "Settings.permissions.users.firstname": "Vorname",
  "Settings.permissions.users.form.sso": "Mit SSO einloggen",
  "Settings.permissions.users.sso.provider.error": "Bei der Abfrage der SSO-Einstellungen ist ein Fehler aufgetreten",
  "Settings.permissions.users.form.sso.description": "Nutzer können sich per SSO einloggen wenn aktiviert (ON)",
  "Settings.permissions.users.inactive": "Inaktiv",
  "Settings.permissions.users.lastname": "Nachname",
  "Settings.permissions.users.listview.header.subtitle": "Alle Nutzer mit Zugriff auf diese Administrationsoberfläche",
  "Settings.permissions.users.roles": "Rollen",
  "Settings.permissions.users.strapi-author": "Autor",
  "Settings.permissions.users.strapi-editor": "Editor",
  "Settings.permissions.users.strapi-super-admin": "Super Admin",
  "Settings.permissions.users.tabs.label": "Tab Berechtigungen",
  "Settings.permissions.users.user-status": "Benutzerstatus",
  "Settings.permissions.users.username": "Benutzername",
  "Settings.profile.form.notify.data.loaded": "Deine Profildaten wurden geladen",
  "Settings.profile.form.section.experience.clear.select": "Ausgewählte Sprache der Oberfläche zurücksetzen",
  "Settings.profile.form.section.experience.here": "hier",
  "Settings.profile.form.section.experience.interfaceLanguage": "Sprache der Oberfläche",
  "Settings.profile.form.section.experience.interfaceLanguage.hint": "Dies wird die Oberfläche für dich in der ausgewählten Sprache darstellen.",
  "Settings.profile.form.section.experience.interfaceLanguageHelp": "Diese Einstellungen werden nur für dich angewandt. Mehr Informationen dazu {here}.",
  "Settings.profile.form.section.experience.mode.hint": "Zeigt die Oberfläche im gewählten Modus.",
  "Settings.profile.form.section.experience.mode.label": "Modus der Oberfläche",
  "Settings.profile.form.section.experience.mode.option-label": "{name}-Modus",
  "Settings.profile.form.section.experience.mode.option-system-label": "Systemeinstellungen verwenden",
  "Settings.profile.form.section.experience.title": "Bedienung",
  "Settings.profile.form.section.head.title": "Nutzerprofil",
  "Settings.profile.form.section.profile.page.title": "Profil-Seite",
  "Settings.roles.create.description": "Die Berechtigungen einer Rolle festlegen",
  "Settings.roles.create.title": "Rolle erstellen",
  "Settings.roles.created": "Rolle erstellt",
  "Settings.roles.edit.title": "Rolle bearbeiten",
  "Settings.roles.form.button.users-with-role": "Benutzer mit dieser Rolle",
  "Settings.roles.form.created": "Erstellt",
  "Settings.roles.form.description": "Name und Beschreibung der Rolle",
  "Settings.roles.form.permission.property-label": "{label} Berechtigungen",
  "Settings.roles.form.permissions.attributesPermissions": "Felderberechtigungen",
  "Settings.roles.form.permissions.create": "Erstellen",
  "Settings.roles.form.permissions.delete": "Löschen",
  "Settings.roles.form.permissions.publish": "Veröffentlichen",
  "Settings.roles.form.permissions.read": "Lesen",
  "Settings.roles.form.permissions.update": "Ändern",
  "Settings.roles.list.button.add": "Neue Rolle hinzufügen",
  "Settings.roles.list.description": "Liste der Rollen",
  "Settings.roles.title.singular": "Rolle",
  "Settings.sso.description": "Einstellungen des Single Sign-On Features konfigurieren.",
  "Settings.sso.form.defaultRole.description": "Die Standard-Rolle wird dem neu erstellten Nutzer zugewiesen",
  "Settings.sso.form.defaultRole.description-not-allowed": "Du hast keine Lese-Berechtigung für die Admin-Rollen",
  "Settings.sso.form.defaultRole.label": "Standard-Rolle",
  "Settings.sso.form.localAuthenticationLock.label": "Lokale Authentifizierungssperre",
  "Settings.sso.form.localAuthenticationLock.description": "Wähle die Rollen aus, für die du die lokale Authentifizierung deaktivieren möchtest",
  "Settings.sso.form.registration.description": "Neuen Nutzer beim Einloggen über SSO erstellen wenn noch kein Account existiert",
  "Settings.sso.form.registration.label": "Auto-Registrierung",
  "Settings.sso.title": "Single Sign-On",
  "Settings.sso.not-available": "SSO ist nur als Teil eines kostenpflichtigen Plans verfügbar. Führe ein Upgrade durch, um zusätzliche Anmelde- und Anmeldemethoden für dein Administrationspanel zu konfigurieren.",
  "Settings.content-history.title": "Inhalts-Historie",
  "Settings.content-history.description": "Erhalte mehr Kontrolle über jeden Schritt des Lebenszyklus deiner Inhalte.",
  "Settings.content-history.not-available": "Inhalts-Historie ist nur als Teil eines kostenpflichtigen Plans verfügbar. Upgrade, um die volle Kontrolle über den Lebenszyklus deiner Inhalte zu erhalten.",
  "Settings.tokens.Button.cancel": "Abbrechen",
  "Settings.tokens.Button.regenerate": "Neugenerieren",
  "Settings.tokens.ListView.headers.createdAt": "Erstellt am",
  "Settings.tokens.ListView.headers.description": "Beschreibung",
  "Settings.tokens.ListView.headers.lastUsedAt": "Zuletzt verwendet",
  "Settings.tokens.ListView.headers.name": "Name",
  "Settings.tokens.RegenerateDialog.title": "Token neugenerieren",
  "Settings.tokens.copy.editMessage": "Aus Sicherheitsgründen kannst du deinen Token nur einmal sehen.",
  "Settings.tokens.copy.editTitle": "Auf diesen Token kann nicht mehr zugegriffen werden.",
  "Settings.tokens.copy.title": "Token",
  "Settings.tokens.copy.lastWarning": "Stell sicher, dass dieser Token kopiert wurde, da du ihn nicht noch einmal sehen können wirst!",
  "Settings.tokens.duration.30-days": "30 Tage",
  "Settings.tokens.duration.7-days": "7 Tage",
  "Settings.tokens.duration.90-days": "90 Tage",
  "Settings.tokens.duration.expiration-date": "Verfallsdatum",
  "Settings.tokens.duration.unlimited": "Unbegrenzt",
  "Settings.tokens.form.description": "Beschreibung",
  "Settings.tokens.form.duration": "Token-Dauer",
  "Settings.tokens.form.name": "Name",
  "Settings.tokens.form.type": "Token-Typ",
  "Settings.tokens.notification.copied": "Token wurde in die Zwischenablage kopiert.",
  "Settings.tokens.popUpWarning.message": "Bist du sicher, dass du den Token neu generieren möchtest?",
  "Settings.tokens.regenerate": "Neugenerieren",
  "Settings.tokens.types.custom": "Benutzerdefiniert",
  "Settings.tokens.types.full-access": "Voller Zugriff",
  "Settings.tokens.types.read-only": "Nur Lesezugriff",
  "Settings.transferTokens.ListView.headers.type": "Token-Typ",
  "Settings.transferTokens.addFirstToken": "Füge deinen ersten Transfer-Token hinzu",
  "Settings.transferTokens.addNewToken": "Neuen Transfer-Token hinzufügen",
  "Settings.transferTokens.create": "Neuen Transfer-Token erstellen",
  "Settings.transferTokens.createPage.title": "Transfer-Token erstellen",
  "Settings.transferTokens.description": "Liste der generierten Transfer-Tokens",
  "Settings.transferTokens.emptyStateLayout": "Du hast noch keinen Inhalt...",
  "Settings.transferTokens.title": "Transfer-Token",
  "Settings.webhooks.create": "Webhook erstellen",
  "Settings.webhooks.create.header": "Neuen Header erstellen",
  "Settings.webhooks.created": "Webhook erstellt",
  "Settings.webhooks.event.publish-tooltip": "Entwurf/Veröffentlichen muss für dieses Event aktiviert sein",
  "Settings.webhooks.event.select": "Auswählen",
  "Settings.webhooks.events.isLoading": "Lädt...",
  "Settings.webhooks.events.create": "Erstellen",
  "Settings.webhooks.events.update": "Aktualisieren",
  "Settings.webhooks.events.delete": "Löschen",
  "Settings.webhooks.form.events": "Events",
  "Settings.webhooks.form.headers": "Header",
  "Settings.webhooks.form.url": "Url",
  "Settings.webhooks.headers.remove": "Header {number} entfernen",
  "Settings.webhooks.key": "Key",
  "Settings.webhooks.list.button.add": "Neuen Webhook hinzufügen",
  "Settings.webhooks.list.description": "POST-Benachrichtigungen bei Änderungen empfangen.",
  "Settings.webhooks.list.empty.description": "Ersten dieser Liste hinzufügen.",
  "Settings.webhooks.list.empty.link": "Siehe unsere Dokumentation",
  "Settings.webhooks.list.empty.title": "Noch keine Webhooks",
  "Settings.webhooks.list.th.actions": "Aktionen",
  "Settings.webhooks.list.th.status": "Status",
  "Settings.webhooks.list.loading.success": "Webhooks wurden geladen",
  "Settings.webhooks.singular": "Webhook",
  "Settings.webhooks.title": "Webhooks",
  "Settings.webhooks.to.delete": "{webhooksToDeleteLength, plural, one {# Datei} other {# Dateien}} ausgewählt",
  "Settings.webhooks.trigger": "Trigger",
  "Settings.webhooks.trigger.cancel": "Trigger abbrechen",
  "Settings.webhooks.trigger.pending": "Ausstehend ...",
  "Settings.webhooks.trigger.save": "Zur Ausführung speichern",
  "Settings.webhooks.trigger.success": "Erfolg!",
  "Settings.webhooks.trigger.success.label": "Trigger war erfolgreich",
  "Settings.webhooks.trigger.test": "Test-Trigger",
  "Settings.webhooks.trigger.title": "Speichere vor Trigger",
  "Settings.webhooks.value": "Wert",
  "Settings.webhooks.validation.name.required": "Name ist erforderlich",
  "Settings.webhooks.validation.name.regex": "Der Name muss mit einem Buchstaben beginnen und darf nur Buchstaben, Zahlen, Leerzeichen und Unterstriche enthalten",
  "Settings.webhooks.validation.url.required": "Url ist erforderlich",
  "Settings.webhooks.validation.url.regex": "Der Wert muss eine gültige Url sein",
  "Settings.webhooks.validation.key": "Schlüssel ist erforderlich",
  "Settings.webhooks.validation.value": "Wert ist erforderlich",
  "Settings.page.PurchaseAudit-logs.description": "Änderungen mit deinem Team verfolgen und überprüfen",
  "Settings.page.PurchaseAudit-logs.perks1": "Änderungen einfach nachverfolgen",
  "Settings.page.PurchaseAudit-logs.perks2": "Änderungen einfach überprüfen",
  "Settings.page.PurchaseAudit-logs.perks3": "Sicherheit und Compliance gewährleisten",
  "Settings.page.PurchaseContent-history.description": "Inhaltsänderungen sofort rückgängig machen",
  "Settings.page.PurchaseContent-history.perks1": "Inhalts-Historie durchsuchen",
  "Settings.page.PurchaseContent-history.perks2": "Änderungen mit einem Klick rückgängig machen",
  "Settings.page.PurchaseContent-history.perks3": "Änderungen über Sprachversionen hinweg verfolgen",
  "Settings.page.PurchaseSSO.description": "Authentifizierung für dein Team vereinfachen",
  "Settings.page.PurchaseSSO.perks1": "Einheitliche Authentifizierung",
  "Settings.page.PurchaseSSO.perks2": "Erhöhte Sicherheit",
  "Settings.page.PurchaseSSO.perks3": "Unterstützung für Webhooks",
  "Settings.page.purchase.upgrade.cta": "Upgrade",
  "Settings.page.purchase.learn-more.cta": "Mehr erfahren",
  "Usecase.back-end": "Backend-Entwickler",
  "Usecase.button.skip": "Diese Frage überspringen",
  "Usecase.content-creator": "Content Creator",
  "Usecase.front-end": "Frontend-Entwickler",
  "Usecase.full-stack": "Full-stack-Entwickler",
  "Usecase.input.work-type": "Welche Rolle beschreibt deine Position am besten?",
  "Usecase.notification.success.project-created": "Projekt wurde erfolgreich erstellt",
  "Usecase.other": "Sonstige",
  "Usecase.title": "Erzähle uns etwas mehr von dir",
  Username,
  Users,
  "Users & Permissions": "Benutzer & Berechtigungen",
  "Users.components.List.empty": "Noch keine Nutzer…",
  "Users.components.List.empty.withFilters": "Es gibt keine Nutzer die den Filtern entsprechen...",
  "Users.components.List.empty.withSearch": "Es gibt keine Nutzer die der Suche ({search}) entsprechen...",
  anErrorOccurred,
  noPreview,
  "app.component.CopyToClipboard.label": "In Zwischenablage kopieren",
  "app.component.search.label": "Suche nach {target}",
  "app.component.table.duplicate": "Dupliziere {target}",
  "app.component.table.edit": "Bearbeite {target}",
  "app.component.table.read": "Lese {target}",
  "app.component.table.select.one-entry": "Selektiere {target}",
  "app.component.table.view": "{target} Details",
  "app.components.BlockLink.blog": "Blog",
  "app.components.BlockLink.blog.content": "Lies die neuesten Nachrichten über Strapi und das Ökosystem.",
  "app.components.BlockLink.cloud": "Strapi Cloud",
  "app.components.BlockLink.cloud.content": "Vollständig verwaltetes Cloud-Hosting für dein Strapi-Projekt.",
  "app.components.BlockLink.code": "Code-Beispiele",
  "app.components.BlockLink.code.content": "Lerne duch das Testen von echten Projekten, die von der Community entwickelt wurden.",
  "app.components.BlockLink.documentation.content": "Entdecke grundlegende Konzepte, Anleitungen und Anweisungen.",
  "app.components.BlockLink.tutorial": "Tutorials",
  "app.components.BlockLink.tutorial.content": "Folge Schritt-für-Schritt Anleitungen die zeigen, wie du Strapi nutzen und anpassen kannst.",
  "app.components.Button.cancel": "Abbrechen",
  "app.components.Button.confirm": "Bestätigen",
  "app.components.Button.reset": "Zurücksetzen",
  "app.components.ComingSoonPage.comingSoon": "Bald verfügbar",
  "app.components.ConfirmDialog.title": "Bestätigung",
  "app.components.DownloadInfo.download": "Download wird ausgeführt...",
  "app.components.DownloadInfo.text": "Dies könnte kurz dauern. Danke für deine Geduld.",
  "app.components.EmptyAttributes.title": "Bisher gibt es noch keine Felder",
  "app.components.EmptyStateLayout.content-document": "Kein Inhalt gefunden",
  "app.components.EmptyStateLayout.content-permissions": "Du hast keine ausreichenden Berechtigungen, um auf diesen Inhalt zuzugreifen",
  "app.components.FreeTrialEndedModal.button.downgrade": "Zu Community Version downgraden",
  "app.components.FreeTrialEndedModal.button.upgrade": "Beim Growth-Plan bleiben",
  "app.components.FreeTrialEndedModal.description": "Dein Zugang zu Growth-Plan-Features wie Inhalts-Historie, Releases und Single Sign-On (SSO) ist abgelaufen.",
  "app.components.FreeTrialEndedModal.notice.item1": "Ein Downgrade entfernt den Zugang zu den oben genannten Features.",
  "app.components.FreeTrialEndedModal.notice.item2": "Die Dokumentversions-Historie wird gelöscht.",
  "app.components.FreeTrialEndedModal.notice.item3": "Alle Releases werden gelöscht.",
  "app.components.FreeTrialEndedModal.notice.item4": "Falls du ein Downgrade durchführst, stelle sicher, dass du ein Root-Admin-Passwort setzt, um den Zugang zum Admin-Panel zu behalten.",
  "app.components.FreeTrialEndedModal.notice.title": "Wichtig zu wissen:",
  "app.components.FreeTrialEndedModal.title": "Deine Testphase ist beendet",
  "app.components.FreeTrialWelcomeModal.button": "Beginne Testphase",
  "app.components.FreeTrialWelcomeModal.description1": "Für die nächsten 30 Tage hast du vollen Zugang zu erweiterten Features wie Inhalts-Historie, Releases und Single Sign-On (SSO) – alles was du brauchst, um die Macht von Strapi CMS zu erkunden.",
  "app.components.FreeTrialWelcomeModal.description2": "Nutze diese Zeit, um deine Inhalts-Workflows mit vollständiger Flexibilität zu erstellen, anzupassen und zu testen!",
  "app.components.FreeTrialWelcomeModal.title": "Wir freuen uns, dich an Bord zu haben",
  "app.components.GuidedTour.CM.create.content": '<p>Erstelle und verwalte all deinen Inhalt hier im Content Manager.</p><p>Beispiel: Im Beispiel der Blog-Website, kann man einen Artikel schreiben, speichern und veröffentlichen.</p><p>💡 Kleiner Tipp: Vergiss nicht, beim Inhalt den du erstellst, "Veröffentlichen" zu drücken.</p>',
  "app.components.GuidedTour.CM.create.title": "⚡️ Erstelle Inhalt",
  "app.components.GuidedTour.CM.success.content": "<p>Super, ein letzter Schritt noch!</p><b>🚀  Sehe deinen Inhalt in Aktion</b>",
  "app.components.GuidedTour.CM.success.cta.title": "Teste die API",
  "app.components.GuidedTour.CM.success.title": "Schritt 2: Abgeschlossen ✅",
  "app.components.GuidedTour.CTB.create.content": "<p>Sammlungen helfen dir, mehrere Einträge zu managen, während Einzel-Einträge dazu da sind, nur einen Eintrag zu managen.</p> <p>Bsp: Für eine Blog-Website wären die Artikel eine Sammlung, während die Homepage ein Einzel-Eintrag wäre.</p>",
  "app.components.GuidedTour.CTB.create.cta.title": "Einen Inhaltstyp bauen",
  "app.components.GuidedTour.CTB.create.title": "🧠 Erstelle deinen ersten Inhaltstyp",
  "app.components.GuidedTour.CTB.success.content": "<p>Sehr gut!</p><b>⚡️ Was willst du mit der Welt teilen?</b>",
  "app.components.GuidedTour.CTB.success.title": "Schritt 1: Abgeschlossen ✅",
  "app.components.GuidedTour.apiTokens.create.content": "<p>Generiere einen Authentifizierungs-Token hier und greife auf den Inhalt, den du gerade erstellt hast, zu.</p>",
  "app.components.GuidedTour.apiTokens.create.cta.title": "Generiere einen API-Token",
  "app.components.GuidedTour.apiTokens.create.title": "🚀 Sehe Inhalt in Aktion",
  "app.components.GuidedTour.apiTokens.success.content": "<p>Sehe Inhalt in Aktion indem du einen HTTP-Request machst:</p><ul><li><p>Zu dieser URL: <light>https://'<'YOUR_DOMAIN'>'/api/'<'YOUR_CT'>'</light></p></li><li><p>Mit dem Header: <light>Authorization: bearer '<'YOUR_API_TOKEN'>'</light></p></li></ul><p>Mehr dazu, wie du mit deinem Inhalt interagierst, liest du in der <documentationLink>Dokumentation</documentationLink>.</p>",
  "app.components.GuidedTour.apiTokens.success.cta.title": "Zurück zur Startseite",
  "app.components.GuidedTour.apiTokens.success.title": "Schritt 3: Abgeschlossen ✅",
  "app.components.GuidedTour.create-content": "Erstelle Inhalt",
  "app.components.GuidedTour.home.CM.title": "⚡️ Was willst du mit der Welt teilen?",
  "app.components.GuidedTour.home.CTB.cta.title": "Gehe zum Inhaltstyp-Editor",
  "app.components.GuidedTour.home.CTB.title": "🧠 Baue die Struktur des Inhalts",
  "app.components.GuidedTour.home.apiTokens.cta.title": "Teste die API",
  "app.components.GuidedTour.skip": "Tour überspringen",
  "app.components.GuidedTour.title": "3 Schritte zum Loslegen",
  "app.components.ImgPreview.hint": "Ziehe eine Datei hierher oder {browse} eine Datei zum hochladen aus",
  "app.components.ImgPreview.hint.browse": "wähle",
  "app.components.InputFile.newFile": "Neue Datei hinzufügen",
  "app.components.InputFileDetails.open": "In einem neuen Tab öffnen",
  "app.components.InputFileDetails.originalName": "Original Name:",
  "app.components.InputFileDetails.remove": "Entferne diese Datei",
  "app.components.InputFileDetails.size": "Größe:",
  "app.components.InstallPluginPage.Download.description": "Es kann einige Sekunden dauern, bis das Plugin heruntergeladen und installiert ist.",
  "app.components.InstallPluginPage.Download.title": "Herunterladen...",
  "app.components.InstallPluginPage.description": "Erweitere deine App ganz einfach.",
  "app.components.LeftMenu.collapse": "Navigationsleiste einklappen",
  "app.components.LeftMenu.expand": "Navigationsleiste ausklappen",
  "app.components.LeftMenu.general": "Generell",
  "app.components.LeftMenu.logo.alt": "Anwendungs-Logo",
  "app.components.LeftMenu.logout": "Abmelden",
  "app.components.LeftMenu.navbrand.title": "Strapi Dashboard",
  "app.components.LeftMenu.navbrand.workplace": "Arbeitsplatz",
  "app.components.LeftMenu.plugins": "Plugins",
  "app.components.LeftMenu.trialCountdown.endedAt": "Deine Testphase endete am {date}.",
  "app.components.LeftMenu.trialCountdown.endsAt": "Deine Testphase endet am {date}.",
  "app.components.LeftMenuFooter.help": "Hilfe",
  "app.components.LeftMenuFooter.poweredBy": "Powered by ",
  "app.components.LeftMenuLinkContainer.collectionTypes": "Sammlungen",
  "app.components.LeftMenuLinkContainer.configuration": "Konfiguration",
  "app.components.LeftMenuLinkContainer.general": "Allgemein",
  "app.components.LeftMenuLinkContainer.noPluginsInstalled": "Noch keine Plugins installiert",
  "app.components.LeftMenuLinkContainer.plugins": "Plugins",
  "app.components.LeftMenuLinkContainer.singleTypes": "Einzel-Einträge",
  "app.components.ListPluginsPage.deletePlugin.description": "Das Deinstallieren des Plugins kann einen Augenblick dauern.",
  "app.components.ListPluginsPage.deletePlugin.title": "Deinstalliere",
  "app.components.ListPluginsPage.description": "Liste aller im Projekt installierten Plugins.",
  "app.components.ListPluginsPage.head.title": "Plugins anzeigen",
  "app.components.Logout.logout": "Ausloggen",
  "app.components.Logout.profile": "Profil",
  "app.components.NotFoundPage.back": "Zurück zur Homepage",
  "app.components.NotFoundPage.description": "Nicht gefunden",
  "app.components.NpsSurvey.banner-title": "Wie wahrscheinlich ist es, dass du Strapi einem Freund oder Kollegen empfiehlst?",
  "app.components.NpsSurvey.feedback-response": "Vielen Dank für dein Feedback!",
  "app.components.NpsSurvey.feedback-question": "Hast du Verbesserungsvorschläge?",
  "app.components.NpsSurvey.submit-feedback": "Feedback senden",
  "app.components.NpsSurvey.dismiss-survey-label": "Umfrage schließen",
  "app.components.NpsSurvey.no-recommendation": "Überhaupt nicht wahrscheinlich",
  "app.components.NpsSurvey.happy-to-recommend": "Sehr wahrscheinlich",
  "app.components.NpsSurvey.select-rating": "Bewertung auswählen",
  "app.components.Official": "Offiziell",
  "app.components.Onboarding.help.button": "Hilfe-Button",
  "app.components.Onboarding.label.completed": "% abgeschlossen",
  "app.components.Onboarding.link.build-content": "Eine Inhaltsarchitektur aufbauen",
  "app.components.Onboarding.link.manage-content": "Inhalte hinzufügen und verwalten",
  "app.components.Onboarding.link.manage-media": "Medien verwalten",
  "app.components.Onboarding.link.more-videos": "Weitere Videos ansehen",
  "app.components.Onboarding.title": "Videos zum Einstieg",
  "app.components.PluginCard.Button.label.download": "Download",
  "app.components.PluginCard.Button.label.install": "Bereits installiert",
  "app.components.PluginCard.PopUpWarning.install.impossible.autoReload.needed": "Die AutoReload-Funktion muss nicht aktiviert sein. Bitte die App mit `yarn develop` starten.",
  "app.components.PluginCard.PopUpWarning.install.impossible.confirm": "Ich verstehe, dass",
  "app.components.PluginCard.PopUpWarning.install.impossible.environment": "Aus Sicherheitsgründen kann ein Plugin nur in einer Entwicklungsumgebung heruntergeladen werden.",
  "app.components.PluginCard.PopUpWarning.install.impossible.title": "Das Herunterladen ist nicht möglich.",
  "app.components.PluginCard.compatible": "Mit dieser App kompatibel",
  "app.components.PluginCard.compatibleCommunity": "Mit der Community kompatibel",
  "app.components.PluginCard.more-details": "Mehr Details",
  "app.components.ToggleCheckbox.off-label": "Nein",
  "app.components.ToggleCheckbox.on-label": "Ja",
  "app.components.ToggleCheckbox.enabled-label": "Aktiviert",
  "app.components.ToggleCheckbox.disabled-label": "Deaktiviert",
  "app.components.UpsellBanner.button": "Jetzt upgraden",
  "app.components.UpsellBanner.intro": "Zugang zu Growth-Plan-Features: ",
  "app.components.UpsellBanner.text": "Als Teil deiner Testphase kannst du Premium-Tools wie Inhalts-Historie, Releases und Single Sign-On (SSO) erkunden.",
  "app.components.Users.MagicLink.connect": "Diesen Link dem Benutzer zum Registrieren schicken.",
  "app.components.Users.MagicLink.connect.sso": "Sende dem Benutzer diesen Link, das erste Login kann über einen SSO-Anbeiter gemacht werden",
  "app.components.Users.ModalCreateBody.block-title.details": "Details",
  "app.components.Users.ModalCreateBody.block-title.roles": "Rolle des Benutzers",
  "app.components.Users.ModalCreateBody.block-title.roles.description": "Dein Benutzer kann eine oder mehrere Rollen haben",
  "app.components.Users.SortPicker.button-label": "Sortieren nach",
  "app.components.Users.SortPicker.sortby.email_asc": "E-Mail (A nach Z)",
  "app.components.Users.SortPicker.sortby.email_desc": "E-Mail (Z nach A)",
  "app.components.Users.SortPicker.sortby.firstname_asc": "Vorname (A nach Z)",
  "app.components.Users.SortPicker.sortby.firstname_desc": "Vorname (Z nach A)",
  "app.components.Users.SortPicker.sortby.lastname_asc": "Nachname (A nach Z)",
  "app.components.Users.SortPicker.sortby.lastname_desc": "Nachname (Z nach A)",
  "app.components.Users.SortPicker.sortby.username_asc": "Benutzername (A nach Z)",
  "app.components.Users.SortPicker.sortby.username_desc": "Benutzername (Z nach A)",
  "app.components.listPlugins.button": "Neues Plugin hinzufügen",
  "app.components.listPlugins.title.none": "Es ist kein Plugin installiert",
  "app.components.listPluginsPage.deletePlugin.error": "Beim Entfernen des Plugins ist ein Fehler aufgetreten",
  "app.containers.App.notification.error.init": "Beim Aufruf der API ist ein Fehler aufgetreten.",
  "app.containers.AuthPage.ForgotPasswordSuccess.text.contact-admin": "Bitte den Administrator kontaktieren, sollte der Link nicht ankommen.",
  "app.containers.AuthPage.ForgotPasswordSuccess.text.email": "Es kann ein paar Minuten dauern, bis der Wiederherstellungslink ankommt.",
  "app.containers.AuthPage.ForgotPasswordSuccess.title": "E-Mail versendet",
  "app.containers.Users.EditPage.form.active.label": "Aktiv",
  "app.containers.Users.EditPage.header.label": "Bearbeite {name}",
  "app.containers.Users.EditPage.header.label-loading": "Bearbeite Nutzer",
  "app.containers.Users.EditPage.roles-bloc-title": "Zugewiesene Rollen",
  "app.containers.Users.ModalForm.footer.button-success": "Nutzer erstellen",
  "app.links.configure-view": "Anzeige konfigurieren",
  "app.page.not.found": "Oh nein! Wir konnten die Seite, nach der du suchst, nicht finden...",
  "app.static.links.cheatsheet": "CheatSheet",
  "app.utils.SelectOption.defaultMessage": " ",
  "app.utils.add-filter": "Filter hinzufügen",
  "app.utils.close-label": "Schließen",
  "app.utils.defaultMessage": " ",
  "app.utils.delete": "Löschen",
  "app.utils.duplicate": "Duplizieren",
  "app.utils.edit": "Bearbeiten",
  "app.utils.errors.file-too-big.message": "Datei ist zu groß",
  "app.utils.filter-value": "Filter-Wert",
  "app.utils.filters": "Filter",
  "app.utils.notify.data-loaded": "{target} wurde geladen",
  "app.utils.placeholder.defaultMessage": " ",
  "app.utils.publish": "Veröffentlichen",
  "app.utils.refresh": "Aktualisieren",
  "app.utils.select-all": "Alle auswählen",
  "app.utils.select-field": "Feld auswählen",
  "app.utils.select-filter": "Filter auswählen",
  "app.utils.unpublish": "Nicht veröffentlichen",
  "app.utils.published": "Veröffentlicht",
  "app.utils.ready-to-publish": "Bereit zum Veröffentlichen",
  "app.utils.already-published": "Bereits veröffentlicht",
  "app.utils.ready-to-publish-changes": "Bereit, Änderungen zu veröffentlichen",
  "app.utils.ready-to-unpublish-changes": "Bereit zum Zurückziehen der Änderungen",
  "app.confirm.body": "Bist du sicher?",
  clearLabel,
  "coming.soon": "Dieser Inhalt ist aktuell unter Bearbeitung und wird in ein paar Wochen zurück sein!",
  "component.Input.error.validation.integer": "Der Wert muss eine Ganzzahl sein",
  "components.AutoReloadBlocker.description": "Strapi mit einem der folgenden Befehle ausführen:",
  "components.AutoReloadBlocker.header": "Dieses Plugin benötigt das Neuladen-Feature.",
  "components.ErrorBoundary.title": "Etwas ist falsch gelaufen...",
  "components.FilterOptions.FILTER_TYPES.$contains": "enthält",
  "components.FilterOptions.FILTER_TYPES.$containsi": "enthält (Groß- und Kleinschreibung wird nicht beachtet)",
  "components.FilterOptions.FILTER_TYPES.$endsWith": "endet mit",
  "components.FilterOptions.FILTER_TYPES.$endsWithi": "endet mit (Groß- und Kleinschreibung wird nicht beachtet)",
  "components.FilterOptions.FILTER_TYPES.$eq": "ist",
  "components.FilterOptions.FILTER_TYPES.$eqi": "ist (Groß- und Kleinschreibung wird nicht beachtet)",
  "components.FilterOptions.FILTER_TYPES.$gt": "ist größer als",
  "components.FilterOptions.FILTER_TYPES.$gte": "is größer als oder gleich",
  "components.FilterOptions.FILTER_TYPES.$lt": "is kleiner als",
  "components.FilterOptions.FILTER_TYPES.$lte": "is kleiner als oder gleich",
  "components.FilterOptions.FILTER_TYPES.$ne": "ist nicht",
  "components.FilterOptions.FILTER_TYPES.$nei": "ist nicht (Groß- und Kleinschreibung wird nicht beachtet)",
  "components.FilterOptions.FILTER_TYPES.$notContains": "enthält nicht",
  "components.FilterOptions.FILTER_TYPES.$notContainsi": "enthält nicht (Groß- und Kleinschreibung wird nicht beachtet)",
  "components.FilterOptions.FILTER_TYPES.$notNull": "ist nicht null",
  "components.FilterOptions.FILTER_TYPES.$null": "ist null",
  "components.FilterOptions.FILTER_TYPES.$startsWith": "startet mit",
  "components.FilterOptions.FILTER_TYPES.$startsWithi": "startet mit (Groß- und Kleinschreibung wird nicht beachtet)",
  "components.Input.error.attribute.key.taken": "Dieser Wert existiert bereits",
  "components.Input.error.attribute.sameKeyAndName": "Darf nicht gleich sein",
  "components.Input.error.attribute.taken": "Dieser Feldname ist bereits vergeben",
  "components.Input.error.contain.lowercase": "Das Passwort muss mindestens einen Kleinbuchstaben enthalten",
  "components.Input.error.contain.number": "Das Passwort muss mindestens eine Zahl enthalten",
  "components.Input.error.contain.maxBytes": "Passwort muss weniger als 73 Bytes haben",
  "components.Input.error.contain.uppercase": "Das Passwort muss mindestens einen Großbuchstaben enthalten",
  "components.Input.error.contentTypeName.taken": "Dieser Name existiert bereits",
  "components.Input.error.custom-error": "{errorMessage} ",
  "components.Input.error.password.noMatch": "Passwörter stimmen nicht überein",
  "components.Input.error.validation.combobox.invalid": "Der angegebene Wert ist nicht gültig",
  "components.Input.error.validation.email": "Das ist keine gültige E-Mail-Adresse",
  "components.Input.error.validation.json": "Das entspricht nicht dem JSON-Format",
  "components.Input.error.validation.lowercase": "Ausschließlich Kleinbuchstaben verwenden",
  "components.Input.error.validation.max": "Der Wert ist zu hoch (max: {max}).",
  "components.Input.error.validation.maxLength": "Der Wert ist zu lang (max: {max}).",
  "components.Input.error.validation.min": "Der Wert ist zu niedrig (min: {min}).",
  "components.Input.error.validation.minLength": "Der Wert ist zu kurz (min: {min}).",
  "components.Input.error.validation.minSupMax": "Kann nicht größer sein",
  "components.Input.error.validation.regex": "Der Wert entspricht nicht dem Regex.",
  "components.Input.error.validation.string": "Der Wert ist kein gültiger String.",
  "components.Input.error.validation.required": "Dieser Wert ist erforderlich.",
  "components.Input.error.validation.unique": "Dieser Wert wird bereits verwendet.",
  "components.Input.error.validation.email.withField": "{field} ist keine gültige E-Mail-Adresse",
  "components.Input.error.validation.json.withField": "{field} entspricht nicht dem JSON-Format",
  "components.Input.error.validation.lowercase.withField": "{field} muss ausschließlich aus Kleinbuchstaben bestehen",
  "components.Input.error.validation.max.withField": "{field} ist zu hoch.",
  "components.Input.error.validation.maxLength.withField": "{field} ist zu lang.",
  "components.Input.error.validation.min.withField": "{field} ist zu niedrig.",
  "components.Input.error.validation.minLength.withField": "{field} ist zu kurz.",
  "components.Input.error.validation.minSupMax.withField": "{field} kann nicht größer sein",
  "components.Input.error.validation.regex.withField": "{field} entspricht nicht dem Regex.",
  "components.Input.error.validation.required.withField": "{field} ist erforderlich.",
  "components.Input.error.validation.unique.withField": "{field} wird bereits verwendet.",
  "components.InputSelect.option.placeholder": "Hier wählen",
  "components.ListRow.empty": "Es gibt keine Daten.",
  "components.NotAllowedInput.text": "Keine Berechtigung dieses Feld zu sehen",
  "components.OverlayBlocker.description": "Es wird ein Feature verwendet, das einen Neustart des Servers erfordert. Bitte warte bis der Server neu gestartet wurde.",
  "components.OverlayBlocker.description.serverError": "Der Server sollte neu gestartet sein, bitte Logs im Terminal überprüfen.",
  "components.OverlayBlocker.title": "Warten auf Neustart.....",
  "components.OverlayBlocker.title.serverError": "Der Neustart dauert länger als erwartet.",
  "components.PageFooter.select": "Einträge pro Seite",
  "components.ProductionBlocker.description": "Aus Sicherheitsgründen müssen wir dieses Plugin in anderen Umgebungen deaktivieren.",
  "components.ProductionBlocker.header": "Dieses Plugin ist nur in der Entwicklungsumgebung verfügbar.",
  "components.ViewSettings.tooltip": "Ansichts-Einstellungen",
  "components.TableHeader.sort": "Sortiere nach {label}",
  "components.Blocks.modifiers.bold": "Fett",
  "components.Blocks.modifiers.italic": "Kursiv",
  "components.Blocks.modifiers.underline": "Unterstrichen",
  "components.Blocks.modifiers.strikethrough": "Durchgestrichen",
  "components.Blocks.modifiers.code": "Inline-Code",
  "components.Blocks.link": "Link",
  "components.Blocks.expand": "Ausklappen",
  "components.Blocks.collapse": "Einklappen",
  "components.Blocks.popover.text": "Text",
  "components.Blocks.popover.text.placeholder": "Link-Text eingeben",
  "components.Blocks.popover.link": "Link",
  "components.Blocks.popover.link.placeholder": "Link einfügen",
  "components.Blocks.popover.link.rel": "Rel (optional)",
  "components.Blocks.popover.link.rel.placeholder": "noopener, nofollow, noreferrer",
  "components.Blocks.popover.link.target": "Ziel (optional)",
  "components.Blocks.popover.link.target.placeholder": "_blank, _self, _parent, _top",
  "components.Blocks.popover.link.error": "Bitte gültigen Link eingeben",
  "components.Blocks.popover.remove": "Entfernen",
  "components.Blocks.popover.edit": "Bearbeiten",
  "components.Blocks.blocks.selectBlock": "Block auswählen",
  "components.Blocks.blocks.text": "Text",
  "components.Blocks.blocks.heading1": "Überschrift 1",
  "components.Blocks.blocks.heading2": "Überschrift 2",
  "components.Blocks.blocks.heading3": "Überschrift 3",
  "components.Blocks.blocks.heading4": "Überschrift 4",
  "components.Blocks.blocks.heading5": "Überschrift 5",
  "components.Blocks.blocks.heading6": "Überschrift 6",
  "components.Blocks.blocks.code": "Code-Block",
  "components.Blocks.blocks.quote": "Zitat",
  "components.Blocks.blocks.image": "Bild",
  "components.Blocks.blocks.unorderedList": "Aufzählungsliste",
  "components.Blocks.blocks.orderedList": "Nummerierte Liste",
  "components.Blocks.blocks.code.languageLabel": "Sprache auswählen",
  "components.Blocks.dnd.instruction": "Um Blöcke neu zu ordnen, drücke Command oder Steuerung zusammen mit Umschalt und den Pfeiltasten nach oben oder unten",
  "components.Blocks.dnd.reorder": "{item}, verschoben. Neue Position im Editor: {position}.",
  "components.Wysiwyg.ToggleMode.markdown-mode": "Markdown-Modus",
  "components.Wysiwyg.ToggleMode.preview-mode": "Vorschau-Modus",
  "components.Wysiwyg.collapse": "Einklappen",
  "components.Wysiwyg.blocks.code": "Code",
  "components.Wysiwyg.selectOptions.H1": "Titel H1",
  "components.Wysiwyg.selectOptions.H2": "Überschrift H2",
  "components.Wysiwyg.selectOptions.H3": "Überschrift H3",
  "components.Wysiwyg.selectOptions.H4": "Überschrift H4",
  "components.Wysiwyg.selectOptions.H5": "Überschrift H5",
  "components.Wysiwyg.selectOptions.H6": "Überschrift H6",
  "components.Wysiwyg.selectOptions.title": "Überschrift hinzufügen",
  "components.WysiwygBottomControls.charactersIndicators": "Zeichen",
  "components.WysiwygBottomControls.fullscreen": "Vergrößern",
  "components.WysiwygBottomControls.uploadFiles": "Datei hierher ziehen, {browse} eine Datei zum hochladen auswählen oder aus der Zwischenablage einfügen.",
  "components.WysiwygBottomControls.uploadFiles.browse": "wähle",
  "components.pagination.go-to": "Gehe zu Seite {page}",
  "components.pagination.go-to-next": "Gehe zur nächsten Seite",
  "components.pagination.go-to-previous": "Gehe zur vorherigen Seite",
  "components.pagination.remaining-links": "Und {number} weitere Links",
  "components.popUpWarning.button.cancel": "Nein, abbrechen",
  "components.popUpWarning.button.confirm": "Ja, bestätigen",
  "components.popUpWarning.message": "Wirklich löschen?",
  "components.popUpWarning.title": "Bitte bestätigen",
  dark,
  "form.button.continue": "Weiter",
  "form.button.done": "Fertig",
  "global.actions": "Aktionen",
  "global.auditLogs": "Audit-Protokolle",
  "global.back": "Zurück",
  "global.cancel": "Abbrechen",
  "global.change-password": "Passwort ändern",
  "global.close": "Schließen",
  "global.content-manager": "Inhalts-Manager",
  "global.home": "Startseite",
  "global.continue": "Weiter",
  "global.delete": "Löschen",
  "global.delete-target": "Lösche {target}",
  "global.description": "Beschreibung",
  "global.details": "Details",
  "global.disabled": "Deaktiviert",
  "global.documentation": "Dokumentation",
  "global.enabled": "Aktiviert",
  "global.error": "Etwas ist schiefgelaufen",
  "global.finish": "Fertig",
  "global.last-change.redo": "Letzte Änderung wiederholen",
  "global.last-change.undo": "Letzte Änderung rückgängig machen",
  "global.last-changes.discard": "Letzte Änderungen verwerfen",
  "global.localeToggle.label": "Sprache der Benutzeroberfläche auswählen",
  "global.marketplace": "Marktplatz",
  "global.more": "Mehr",
  "global.name": "Name",
  "global.new": "Neu",
  "global.none": "Keine",
  "global.password": "Passwort",
  "global.plugins": "Plugins",
  "global.plugins.content-manager": "Inhalts-Manager",
  "global.plugins.content-manager.description": "Schnelle Möglichkeit, die Daten in deiner Datenbank zu sehen, zu bearbeiten und zu löschen.",
  "global.plugins.content-type-builder": "Inhaltstyp-Editor",
  "global.plugins.content-type-builder.description": "Modelliere die Datenstruktur deiner API. Erstelle neue Felder und Relationen in nur einer Minute. Die Dateien werden automatisch erstellt und in deinem Projekt aktualisiert.",
  "global.plugins.documentation": "Dokumentation",
  "global.plugins.documentation.description": "Erstelle ein OpenAPI-Dokument und visualisiere deine API mit SWAGGER UI.",
  "global.plugins.email": "E-Mail",
  "global.plugins.email.description": "Konfiguriere deine Anwendung für den Versand von E-Mails.",
  "global.plugins.graphql": "GraphQL",
  "global.plugins.graphql.description": "Fügt GraphQL-Endpunkt mit Standard-API-Methoden hinzu.",
  "global.plugins.i18n": "Internationalisierung",
  "global.plugins.i18n.description": "Dieses Plugin ermöglicht es, Inhalte in verschiedenen Sprachen zu erstellen, zu lesen und zu aktualisieren, sowohl über das Admin Panel als auch über die API.",
  "global.plugins.sentry": "Sentry",
  "global.plugins.sentry.description": "Sende Strapi-Fehlerereignisse an Sentry.",
  "global.plugins.upload": "Mediathek",
  "global.plugins.upload.description": "Verwaltung der Mediendateien.",
  "global.plugins.users-permissions": "Rollen und Berechtigungen",
  "global.plugins.users-permissions.description": "Schütze deine API mit einem vollständigen Authentifizierungsprozess auf Basis von JWT. Dieses Plugin verfügt auch über eine ACL-Strategie, mit der du die Berechtigungen zwischen den Benutzergruppen verwalten kannst.",
  "global.profile": "Profil",
  "global.profile.settings": "Profil-Einstellungen",
  "global.prompt.unsaved": "Seite wirklich verlassen? Alle Änderungen gehen hierdurch verloren.",
  "global.reset-password": "Passwort zurücksetzen",
  "global.roles": "Rollen",
  "global.save": "Speichern",
  "global.search": "Suchen",
  "global.see-more": "Mehr anzeigen",
  "global.select": "Auswählen",
  "global.select-all-entries": "Wähle alle Einträge aus",
  "global.settings": "Einstellungen",
  "global.type": "Typ",
  "global.users": "Benutzer",
  "global.fullname": "{firstname} {lastname}",
  "global.learn-more": "Mehr erfahren",
  light,
  "notification.contentType.relations.conflict": "Inhaltstyp hat Konflikt in Relationen",
  "notification.default.title": "Information:",
  "notification.ee.warning.at-seat-limit.title": "{licenseLimitStatus, select, OVER_LIMIT {Über dem} AT_LIMIT {Am}} Sitzlimit ({currentUserCount}/{permittedSeats})",
  "notification.ee.warning.over-.message": "Sitze hinzufügen um Benutzer {licenseLimitStatus, select, OVER_LIMIT {einzuladen} AT_LIMIT {wieder zu aktivieren}}. Wenn du dies bereits getan hast, es aber noch nicht in Strapi angezeigt wird, musst du deine Anwendung neu starten.",
  "notification.error": "Ein Fehler ist aufgetreten",
  "notification.error.invalid.configuration": "Du hast eine ungültige Konfiguration, überprüfe dein Serverprotokoll für weitere Informationen.",
  "notification.error.layout": "Das Layout konnte nicht abgerufen werden.",
  "notification.error.tokennamenotunique": "Name bereits einem anderen Token zugewiesen",
  "notification.form.error.fields": "Das Formular enthält Fehler",
  "notification.form.success.fields": "Änderungen gespeichert",
  "notification.link-copied": "Link in die Zwischenablage kopiert",
  "notification.permission.not-allowed-read": "Keine Berechtigung dieses Dokument einzusehen",
  "notification.success.apitokencreated": "API-Token erfolgreich erstellt",
  "notification.success.apitokenedited": "API-Token erfolgreich bearbeitet",
  "notification.success.delete": "Eintrag wurde gelöscht",
  "notification.success.saved": "Gespeichert",
  "notification.success.title": "Erfolg:",
  "notification.success.transfertokencreated": "Transfer-Token erfolgreich erstellt",
  "notification.success.transfertokenedited": "Transfer Token erfolgreich bearbeitet",
  "notification.version.update.message": "Eine neue Strapi Version ist verfügbar",
  "notification.warning.404": "404 - Nicht gefunden",
  "notification.warning.title": "Warnung:",
  or,
  "request.error.model.unknown": "Dieses Schema existiert nicht",
  selectButtonTitle,
  skipToContent,
  submit,
  "tours.contentTypeBuilder.Introduction.title": "Inhaltstyp-Editor",
  "tours.contentTypeBuilder.Introduction.content": "Erstelle und verwalte deine Inhaltsstruktur mit Collection Types, Single Types und Komponenten.",
  "tours.contentTypeBuilder.AIChat.title": "Zeit, loszulegen!",
  "tours.contentTypeBuilder.AIChat.content": "<p>Falls du Fragen zum Inhaltstyp-Editor oder Strapi hast, stelle sie hier.</p><p>Strapi AI kann Schemas erstellen, die auf deine Bedürfnisse zugeschnitten sind. Frage nach genau dem, was du willst, zum Beispiel:<ul><li>Datumsauswahl</li><li>E-Mail- und Passwort-Felder</li><li>Medien aller Art</li><li>UIDs</li></ul></p><p>Sei nicht schüchtern, probiere es aus!</p>",
  "tours.contentTypeBuilder.CollectionTypes.title": "Collection Types",
  "tours.contentTypeBuilder.CollectionTypes.content": "Eine Inhaltsstruktur, die mehrere Einträge verwalten kann, wie Artikel oder Produkte.",
  "tours.contentTypeBuilder.SingleTypes.title": "Single Types",
  "tours.contentTypeBuilder.SingleTypes.content": "Eine Inhaltsstruktur, die nur einen Eintrag verwalten kann, wie eine Startseite oder Kontaktseite.",
  "tours.contentTypeBuilder.Components.title": "Komponenten",
  "tours.contentTypeBuilder.Components.content": "Eine Inhaltsstruktur, die in mehreren Collection Types oder Single Types verwendet werden kann.",
  "tours.contentTypeBuilder.YourTurn.title": "Du bist dran — Erstelle etwas!",
  "tours.contentTypeBuilder.YourTurn.content": 'Erstelle einen Sammlungs- oder Einzeltyp. Klicke auf den "+"-Button, wähle einen Namen, klicke auf "Weiter" und wir führen dich von dort weiter.',
  "tours.contentTypeBuilder.AddFields.title": "Füge ein Feld hinzu, um es zum Leben zu erwecken",
  "tours.contentTypeBuilder.AddFields.content": "Beginne mit dem Hinzufügen deines ersten Feldes — wie ein Name, Bild oder eine Relation. Dein Inhaltstyp braucht eine Form, bevor er Inhalte aufnehmen kann.",
  "tours.contentTypeBuilder.Save.title": "Gehe nicht ohne zu speichern!",
  "tours.contentTypeBuilder.Save.content": 'Klicke auf den "Speichern"-Button, um deinen Inhaltstyp zu sichern und deine Arbeit nicht zu verlieren. Fast geschafft!',
  "tours.contentTypeBuilder.Finish.title": "Zeit, Inhalte zu erstellen!",
  "tours.contentTypeBuilder.Finish.content": "Jetzt wo du Inhaltstypen erstellt hast, kannst du Inhalte im Content Manager erstellen.",
  "tours.apiTokens.Introduction.title": "API-Token",
  "tours.apiTokens.Introduction.content": "Erstelle und verwalte API-Token mit hochgradig anpassbaren Berechtigungen.",
  "tours.apiTokens.ManageAPIToken.title": "Ein API-Token verwalten",
  "tours.apiTokens.ManageAPIToken.content": 'Klicke auf das "Stift"-Icon, um ein vorhandenes API-Token anzuzeigen und zu aktualisieren.',
  "tours.apiTokens.ViewAPIToken.title": "API-Token anzeigen",
  "tours.apiTokens.ViewAPIToken.content": 'Klicke auf den "Token anzeigen"-Button, um dein API-Token zu sehen.',
  "tours.apiTokens.CopyAPIToken.title": "Kopiere den neuen API-Token",
  "tours.apiTokens.CopyAPIToken.content": "Stelle sicher, dass du es jetzt machst, du wirst ihn nicht noch einmal sehen können. Du musst einen neuen generieren, wenn du ihn verlierst.",
  "tours.apiTokens.FinalStep.title": "Zeit, deine Anwendung zu deployen!",
  "tours.apiTokens.FinalStep.content": "Deine Anwendung ist bereit deployed zu werden und ihre Inhalte mit der Welt zu teilen!",
  "tours.contentManager.Introduction.title": "Content Manager",
  "tours.contentManager.Introduction.content": "Erstelle und verwalte Inhalte von deinen Collection Types und Single Types.",
  "tours.contentManager.CreateNewEntry.title": "Neuen Eintrag erstellen",
  "tours.contentManager.CreateNewEntry.content": 'Klicke auf den "Neuen Eintrag erstellen"-Button, um einen neuen Eintrag für diesen Sammlungstyp zu erstellen und zu veröffentlichen.',
  "tours.contentManager.Fields.title": "Felder",
  "tours.contentManager.Fields.content": "Füge Inhalte zu den im Inhaltstyp-Editor erstellten Feldern hinzu.",
  "tours.contentManager.Publish.title": "Veröffentlichen",
  "tours.contentManager.Publish.content": "Veröffentliche Einträge, um ihre Inhalte über die Content-API verfügbar zu machen.",
  "tours.contentManager.FinalStep.title": "Zeit, API-Token zu erstellen!",
  "tours.contentManager.FinalStep.content": "Jetzt wo du Inhalte erstellt und veröffentlicht hast, ist es Zeit, API-Token zu erstellen und Berechtigungen einzurichten.",
  "tours.stepCount": "Schritt {currentStep} von {tourLength}",
  "tours.skip": "Überspringen",
  "tours.next": "Weiter",
  "tours.gotIt": "Verstanden",
  "tours.overview.title": "Entdecke deine Anwendung!",
  "tours.overview.subtitle": "Folge der geführten Tour, um das Beste aus Strapi herauszuholen.",
  "tours.overview.close": "Geführte Tour schließen",
  "tours.overview.tasks": "Deine Aufgaben",
  "tours.overview.contentTypeBuilder.label": "Erstelle dein Schema",
  "tours.overview.contentManager.label": "Erstelle und veröffentliche Inhalte",
  "tours.overview.apiTokens.label": "Erstelle und kopiere ein API-Token",
  "tours.overview.strapiCloud.label": "Deploye deine Anwendung zu Strapi Cloud",
  "tours.overview.strapiCloud.link": "Dokumentation lesen",
  "tours.overview.tour.link": "Starten",
  "tours.overview.tour.done": "Fertig",
  "tours.overview.close.description": "Bist du sicher, dass du die geführte Tour schließen möchtest?",
  "tours.profile.title": "Geführte Tour",
  "tours.profile.description": "Du kannst die geführte Tour jederzeit zurücksetzen.",
  "tours.profile.reset": "Geführte Tour zurücksetzen",
  "tours.profile.notification.success.reset": "Geführte Tour zurückgesetzt",
  "tours.overview.completed": "{completed}% abgeschlossen",
  "widget.key-statistics.title": "Projekt-Statistiken",
  "widget.key-statistics.list.admins": "Admins",
  "widget.key-statistics.list.apiTokens": "API-Token",
  "widget.key-statistics.list.assets": "Assets",
  "widget.key-statistics.list.components": "Komponenten",
  "widget.key-statistics.list.contentTypes": "Inhaltstypen",
  "widget.key-statistics.list.entries": "Einträge",
  "widget.key-statistics.list.locales": "Lokalisierungen",
  "widget.key-statistics.list.webhooks": "Webhooks",
  "widget.last-activity.link": "Audit Logs öffnen",
  "widget.last-activity.no-activity": "Keine Aktivität",
  "widget.last-activity.title": "Letzte Aktivität",
  "widget.profile.title": "Profil",
  "HomePage.addWidget.title": "Widget hinzufügen",
  "HomePage.addWidget.noWidgetsAvailable": "Keine Widgets zum Hinzufügen verfügbar",
  "HomePage.addWidget.button": "Widget hinzufügen",
  "HomePage.widget.delete": "Löschen",
  "HomePage.widget.drag": "Ziehen zum Verschieben"
};
export {
  Analytics,
  Documentation,
  Email,
  Password,
  Provider,
  ResetPasswordToken,
  Role,
  Username,
  Users,
  anErrorOccurred,
  clearLabel,
  dark,
  de as default,
  light,
  noPreview,
  or,
  selectButtonTitle,
  skipToContent,
  submit
};
//# sourceMappingURL=de.json-Z72CVQR7.js.map
